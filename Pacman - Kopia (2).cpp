...
string Map_c="111111111111111111111111000000000010000000000110111011110101111011101131110111101011110111311000000011010110000000110111010000000001011101100000101111111010000011111101000010000101111111111011110101111011111\
...
1110101011111110101011113000010000100001000031101111111101011111111011000000000000000000000111111111111111111111111",Map=Map_c,DMap;

/// ======================================================= Structs
struct Pos{float x=11,y=11,d=0,j=0;Pos(float X=11,float Y=11){x=X;y=Y;}};

/// ======================================================= All variables
int M,MEM2,GS,B=0,CT;
Pos PL(1,1),Ghosts[5];
vector<int> Dir;
...
float Cl=clock(),PV=Cl,CR,lag=0.0;
...
///Game loop
    ...
    {
///Engine
        CR=clock();lag+=CR-PV;PV=CR;

///Logic
        while(lag>=0.01)
        {
    //Move entity
            B-=B>0;CH(LEFT,2)CH(RIGHT,3)CH(UP,0)CH(DOWN,1)SomeMove(PL,10000);char&K=Map[PL.x+PL.y*23];if(K==51)B=500000;K=50;
...
///Render
    //Draw map
        ...
        if(Time(Cl,0.05))
        {
            ...
            Loop(i,23)Loop(j,23){M=Map[i*23+j];if(M==49)DMap+=(B>150000?176:(B?177:219));if(M==48){DMap+='.';MEM2++;}if(M==50)DMap+=' ';if(M==51){DMap+=254;MEM2++;}if(j==22)DMap+='\n';}
            SP(0,0);Color(8);cout<<DMap<<"\t\t\t\t\r"<<"Bonus: "<<B/100<<"\tPoints: "<<MEM2;GS=!MEM2;
        }
    //Draw entity
        Color(14);if(B)Color(15);SP(PL);cout<<'O';Loop(i,5){if(!B)Color(10+i);SP(Ghosts[i]);cout<<(B?char(1):char(2));}

///End game
        if(GS==1||GS==2)
        {
            system("cls");cout<<(GS==1?"CONGRATULATIONS!!!\nDo you wanna try again? [Y/N]\n":"GAME OVER.. :(\nDo you wanna try again? [Y/N]\n");
